...
export default function Post(thisPost) {
  ...
  const [isliked, setIsLiked] = useState(
    thisPost?.likes?.includes(user.userName)
  );
  const [expandedCaption, setExpandedCaption] = useState(false);
  const [commentWindow, setCommentWindow] = useState(false);
  const [comment, setComment] = useState("");
  const [isFollowing, setIsFollowing] = useState(
    user?.following.includes(thisPost.userName)
  );
          ...
          <div className={styles.postCard_left}>
            <div className={styles.postCard_left_content}>
              ...
              <div className={styles.userDetailsForMobile}>
                <div className={styles.userInfo_mobile}>
                  <div className={styles.profilePicture_container_mobile}>
                    <img
                      src={`${
                        thisPost?.pfp.includes("cloudinary")
                          ? thisPost.pfp
                          : `${thisPost.pfp}/60`
                      }`}
                      alt="pfp"
                  ...
                  <div
                    className={styles.userName_mobile}
                    onClick={() => `${nav(`/u/${thisPost.userName}`)}`}
                  >{`${thisPost.userName}`}</div>
                  <div
                    className={styles.userInfo_trackName_mobile}
                    onClick={() => `${nav(`/track/${thisPost.trackName}`)}`}
                  >{`${thisPost.trackName}`}</div>
              ...
              <div className={styles.postMedia}>
                <img
                  src={`${
                    thisPost?.mediaPicture.includes("cloudinary")
                      ? thisPost.mediaPicture
                      : `${thisPost.mediaPicture}/1800`
                  }`}
                  alt="image"
                  draggable="false"
                />
              ...
              <div className={`${styles.postInfo} ${styles.not_selectable}`}>
                <div
                  className={`${styles.likes} ${styles.not_selectable} ${
          ...
          <div className={styles.postCard_right}>
            <div className={styles.postCard_right_content}>
              <div className={styles.userInfo}>
                ...
                >{`${thisPost.userName}`}</div>
                <div
                  className={styles.userInfo_trackName}
                  onClick={() =>
                    `${nav(
                      `/track/${thisPost.trackName.split(" ").join("_")}`
                    )}`
                  }
                >{`${thisPost.trackName}`}</div>
              ...
              {commentWindow ? (
                <>
                  <div className={styles.comment_bg_screen}>
                    <div
                      ref={commentRef}
                      className={styles.comment_writing_window}
                    >
                    ...
                    return (
                      <>
                        <div className={styles.commentInfo}>
                          <img
                            src={`${
                              eachComment?.profilePicture.includes("cloudinary")
                                ? eachComment.profilePicture
                                : `${eachComment.profilePicture}/300`
                            }`}
                            alt="pfp"
                          />
                          <div
                            className={styles.CommentuserName}
                          >{`${eachComment.commentContent}`}</div>
                        </div>
                      </>
                    );
                  })}
